...
import {
  ...
  Terminal, Sliders, RefreshCw, Loader2, CheckCircle2
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  async function seed() {
    ...
    try {
      const r = await fetch("/api/seed", {
        method: "POST",
        body: JSON.stringify({ customers: 2500 }),
        headers: seedRequestHeaders()
      });
      const data = await r.json().catch(() => ({}));
      if (!r.ok) {
        setSeedMessage({ type: "err", text: data.error || `Seed failed (${r.status})` });
        return;
      }

      if (data.status === "started") {
        setSeedMessage({ type: "ok", text: "Seeding demo data… dashboard will refresh in a few seconds" });
        for (let i = 0; i < 40; i++) {
          await new Promise((resolve) => setTimeout(resolve, 2000));
          const [fresh, camps] = await Promise.all([fetchOverview(), fetchCampaigns()]);
          await qc.invalidateQueries();
          const nextCount = fresh?.kpis?.customers ?? 0;
          const campCount = fresh?.kpis?.campaigns ?? camps?.length ?? 0;
          if (nextCount > 0 && campCount >= 4 && i >= 1) {
            setSeedMessage({
              type: "ok",
              text: `Demo data refreshed — ${nextCount.toLocaleString("en-IN")} customers, ${campCount} campaigns`
            });
            return;
          }
        }
        setSeedMessage({ type: "ok", text: "Seed started — if numbers look stale, refresh the page in a moment" });
        return;
      }

      setSeedMessage({
        type: "ok",
        text: `Demo data refreshed — ${Number(data.customers ?? 0).toLocaleString("en-IN")} customers, ${Number(data.orders ?? 0).toLocaleString("en-IN")} orders`
      });
      await qc.invalidateQueries();
    } catch {
      setSeedMessage({ type: "err", text: "Seed failed — could not reach the server" });
    } finally {
      setSeeding(false);
    }
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.04] pb-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={seed}
            disabled={seeding}
            className="btn-secondary text-xs h-9 inline-flex items-center gap-1.5 disabled:opacity-60"
          >
      ...
      {seedMessage && (
        <div className={clsx(
          "glass p-3 text-xs flex items-center gap-2",
          seedMessage.type === "ok" ? "border-green-500/30 text-green-300" : "border-red-500/30 text-red-300"
        )}>
          {seedMessage.type === "ok" ? <CheckCircle2 size={14} /> : <AlertTriangle size={14} />}
          {seedMessage.text}
        </div>
      )}
        ...
        >
          {isEventsLoading && !liveEvents ? (
            ...
            <div className="text-center py-8 text-white/20 space-y-1">
              <div>[SYSTEM] Telemetry pipeline clear. Awaiting campaign dispatches.</div>
              <div className="text-[10px] text-white/10">Create a campaign or click Seed Demo Data to populate events.</div>
            </div>
          ) : (
            (() => {
              ...
              if (filtered.length === 0) {
                return (
                  <div className="text-[#8A8A8A] text-center py-4 space-y-1">
                    <div>[SYSTEM] No telemetry matches the &quot;{filterType}&quot; filter.</div>
                    {filterType !== "ALL" && (
                      <button
                        type="button"
