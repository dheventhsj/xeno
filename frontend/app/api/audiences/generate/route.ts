...
import { createCampaignFromSegment } from "@/lib/campaigns";
...
export async function POST(req: Request) {
  ...
  const locationUnavailable = (audience as { locationUnavailable?: string }).locationUnavailable ?? null;
  let campaign = null;

  if (audience.count > 0 && !locationUnavailable) {
    campaign = await createCampaignFromSegment({
  ...
  return NextResponse.json({
    ...
    locationUnavailable,
    campaign: campaign
      ? { id: campaign.id, status: campaign.status, recommendedChannel: campaign.recommendedChannel }
      : null
  });
}
