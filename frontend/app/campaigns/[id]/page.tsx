...
import { DeliveryLifecycleViz } from "@/components/DeliveryLifecycleViz";
...
export default function CampaignDetailPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
              ...
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <span className={clsx(
                  ...
                  c.status === "RUNNING" ? "badge-success"
                    : c.status === "DELIVERED" ? "bg-teal-500/10 border-teal-500/30 text-teal-300"
                    : c.status === "QUEUED" ? "bg-amber-500/10 border-amber-500/30 text-amber-300"
                    : c.status === "COMPLETED" ? "badge-info" : "badge-warning"
                )}>
                  {(c.status === "RUNNING" || c.status === "QUEUED") && (
                    <span className={clsx("live-dot mr-1", c.status === "QUEUED" && "bg-amber-400")} />
                  )}
                  {c.status === "QUEUED" ? "Queued" : c.status === "DELIVERED" ? "Delivered" : c.status}
                </span>
              </div>
      ...
      {(insights?.explainability?.length > 0 || c.recommendedChannel) && (
        <div className="glass p-6 space-y-4">
          <h2 className="text-xs font-semibold text-white/70 uppercase tracking-widest">Why was this selected?</h2>
          <div className="flex flex-wrap gap-2">
            ...
            {c.recommendedChannel && (
              <span className="inline-flex items-center gap-1.5 chip text-[10px] py-1 border-blue-500/20 text-blue-300">
                ✓ Preferred channel = {c.recommendedChannel}
              </span>
            )}
            {c.goal && (
              <span className="inline-flex items-center gap-1.5 chip text-[10px] py-1 border-purple-500/20 text-purple-300">
                ✓ Offer aligned to goal: {c.goal.slice(0, 40)}{c.goal.length > 40 ? "…" : ""}
              </span>
            )}
          </div>
          <div className="grid gap-3 sm:grid-cols-3 text-[10px]">
            <div className="glass-inner p-3">
              <div className="text-[#8A8A8A] uppercase font-semibold mb-1">Why this audience?</div>
              <div className="text-[#CFCFCF]">{c.segment?.description ?? "AI-segmented cohort matching campaign goal"}</div>
            </div>
            <div className="glass-inner p-3">
              <div className="text-[#8A8A8A] uppercase font-semibold mb-1">Why this channel?</div>
              <div className="text-[#CFCFCF]">{insights?.battle?.rationale ?? `${c.recommendedChannel} has highest engagement for this segment`}</div>
            </div>
            <div className="glass-inner p-3">
              <div className="text-[#8A8A8A] uppercase font-semibold mb-1">Why this offer?</div>
              <div className="text-[#CFCFCF]">Personalized message variants A/B/C optimized for {c.recommendedChannel} delivery</div>
            </div>
          </div>
        </div>
      )}
      ...
      {a && a.sent > 0 && (
        <div className="glass p-6">
          <h2 className="text-xs font-semibold text-white/70 uppercase tracking-widest mb-6 flex items-center gap-2">
            <Activity size={12} className="text-emerald-400" /> Communication Lifecycle
          </h2>
          <DeliveryLifecycleViz
            sent={a.sent}
      ...
      <FollowUpRecommendations campaignGoal={c.goal} channel={c.recommendedChannel} status={c.status} />
      ...
      <div className="glass p-6">
        <h2 className="text-xs font-semibold text-white/70 uppercase tracking-widest mb-4 flex items-center gap-2">
          <MessageSquare size={13} /> AI Generated Content
        </h2>
        <MessagePreviewPanel
          goal={c.goal}
          campaignId={id}
          category={c.segment?.name?.includes("Beauty") ? "skincare" : "premium products"}
      ...
      <div className="glass p-6">
        <h2 className="text-xs font-semibold text-white/70 uppercase tracking-widest mb-4 flex items-center gap-2">
          <Terminal size={13} className="text-purple-400" /> AI Execution Logs
        </h2>
        <ExecutionLogPanel />
      </div>
