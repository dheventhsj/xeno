...
import { Target, Sparkles, Users, Loader2, AlertCircle, Megaphone, ArrowRight } from "lucide-react";
export default function AudiencesPage() {
  ...
  const isLocationEmpty = Boolean(result?.locationUnavailable);
  const emptyLocation = result?.locationUnavailable;
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {result && (
        <div className="glass p-6 border-white/[0.08] bg-[#111]/80 space-y-6 animate-scale-in">
          ...
          {result.campaign && (
            <div className="glass-inner p-4 border-amber-500/20 bg-amber-500/[0.04] flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-start gap-3">
                <Megaphone size={16} className="text-amber-400 mt-0.5 shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-white">Campaign started automatically</p>
                  <p className="text-[11px] text-[#8A8A8A] mt-0.5">
                    Status <span className="text-amber-300 font-semibold">Queued</span> on {result.campaign.recommendedChannel} — delivers in ~5s on the Campaign War Room.
                  </p>
                </div>
              </div>
              <Link
                href="/campaigns"
                className="btn-secondary inline-flex items-center gap-1.5 text-[10px] py-1.5 px-3 shrink-0"
              >
          ...
          {isLocationEmpty ? (
            <div className="glass-inner p-8 text-center border-amber-500/20 bg-amber-500/[0.03]">
              <AlertCircle size={28} className="text-amber-400 mx-auto mb-3" />
              <p className="text-sm font-semibold text-white">
                No customers available from {emptyLocation}
              </p>
              <p className="text-xs text-[#8A8A8A] mt-2 max-w-md mx-auto">
                This location is not in our shopper directory. Try Mumbai, Delhi, Bangalore, Chennai, Pune, Hyderabad, Kolkata, Jaipur, Ahmedabad, or Lucknow.
              </p>
            </div>
          ) : (
          <>
