...
export async function seedDatabase(customerCount = 5000) {
  ...
  // Orders + timeline for a sample only (full fan-out times out on serverless)
  const orderSampleSize = Math.min(count, 600);
  const sampleForOrders = await prisma.customer.findMany({
    take: orderSampleSize,
    select: {
      id: true,
      orderCount: true,
  ...
  for (const c of sampleForOrders) {
    const n = Math.min(Math.max(c.orderCount, rand(1, 4)), 4);
    for (let k = 0; k < n; k++) {
      orderBatch.push({
        customerId: c.id,
  ...
  const lite = count >= 1500;
  await seedDemoCampaigns(true, { lite });
...
async function clearDatabaseFast() {
  try {
    ...
    try { await (prisma as any).marketingMemory?.deleteMany?.(); } catch { /* optional */ }
    try { await (prisma as any).agentReasoningLog?.deleteMany?.(); } catch { /* optional */ }
