...
import { filterToPrisma, parseAudiencePrompt, emptyLocationAudience, extractCityFromPrompt } from "./audience";
...
export async function generateAudience(prompt: string) {
  ...
  const heuristic = parseAudiencePrompt(prompt);
  const parsed = llmParsed ?? heuristic;
  ...
  if (!filter.city) {
    const city = extractCityFromPrompt(prompt.toLowerCase());
    if (city) {
      filter.city = city;
      if (!reasoning.some(r => r.toLowerCase().includes(city.toLowerCase()))) {
        reasoning.push(`City: ${city}`);
      }
  ...
  if (filter.city && count === 0) {
    const cityOnlyCount = await prisma.customer.count({
      where: { city: { equals: filter.city, mode: "insensitive" } },
    });
    if (cityOnlyCount === 0) {
      return emptyLocationAudience(filter.city, filter, reasoning, prompt, parseSource);
    }
  }

  const [agg, byCity, byCategory, byChannel] = await Promise.all([
...
export async function planCampaign(goal: string, reasoning: ReasoningChain, onProgress?: (msg: any) => void, sessionId?: string): Promise<{ draft: CampaignDraft; reasoning: ReasoningChain; thinkingTimeline: ReturnType<typeof createThinkingTimeline>; channelBattle: ReturnType<typeof simulateChannelBattle> }> {
  ...
  if (sessionId) {
    await prisma.agentReasoningLog.create({
      data: {
        sessionId,
        goal,
